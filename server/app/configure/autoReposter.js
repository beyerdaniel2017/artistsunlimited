  ...
  if (id) {
    SCR.put('/e1/me/track_reposts/' + id, channel.accessToken, function(err, data) {
      if (err) {
        ...
        if (event.email) {
          sendMessage(event.name, event.email, "Edward Sanchex", "coayscue@artistsunlimited.co", "Music Submission", "Hey " + event.name + ",<br><br>We would just like to let you know the track <a href='" + event.trackURL + "'>" + event.title + "</a> has been reposted on <a href='" + channel.url + "'>" + channel.displayName + "</a>! If you would like to do another round of reposts please resubmit your track to <a href='artistsunlimited.co/submit'>artistsunlimited.co/submit</a>. We will get back to you ASAP and continue to do our best in making our submission process as quick and easy as possible.<br><br>How was this experience by the way? Feel free to email some feedback, suggestions or just positive reviews to feedback@peninsulamgmt.com.<br><br>Edward Sanchez<br> Peninsula MGMT Team <br>www.facebook.com/edwardlatropical");
        }
