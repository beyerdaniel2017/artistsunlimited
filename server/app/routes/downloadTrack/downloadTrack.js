...
router.post('/tasks', function(req, res, next) {
  ...
  if (body.artists) {
    body.artists.forEach(function(artist) {
      scWrapper.request({method: 'PUT', path:'/me/followings/' + artist.id, qs: {}}, function(err, response){
  ...
  if (body.artistID) {
    scWrapper.request({method: 'PUT', path:'/me/followings/' + body.artistID, qs: {}}, function(err, response){
  ...
  if (body.playlists) {
    body.playlists.forEach(function(playlist) {
      ...
      scWrapper.request({method: 'PUT', path:'/e1/me/playlist_likes/' + playlist.id, qs: {}}, function(err, data){
