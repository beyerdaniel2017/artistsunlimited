...
app.controller('ReleaserController', function($scope, posts, StorageFactory, BroadcastFactory, $state, SessionService,$stateParams,$window) {
  ...
  $scope.broadcastPost = function(post){
    ...
    if(isValid){
      ...
      .then(function(res){
        return BroadcastFactory.submitSoundCloudPost(post._id, {awsAudioKeyName: post.awsAudioKeyName})
      })    
      .then(function(res){
        if (post.awsAudioKeyName){
          ...
          var trackFile = new File(res.data.Body.data, post.awsAudioKeyName, {type: 'audio/mp3'});
          SC.upload({
            file: trackFile,
