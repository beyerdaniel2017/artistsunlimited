...
app.controller("AdminReForReInteractionController", function($rootScope, $state, $scope, $http, AuthService, $window, SessionService, socket, $stateParams, trade, p1Events, p2Events, currentTrades,paidReposts) {
...
console.log("queue",$scope.user.queue);
  ...
  $scope.undo = function() {
    $.Zebra_Dialog("Undo changes?", {
      'type': 'confirmation',
      'buttons': [{
        caption: 'Undo',
        callback: function() {
          $http.get('/api/trades/byID/' + $stateParams.tradeID+'/'+PaidUserId)
          .then(function(res) {
            $scope.trade = res.data;
